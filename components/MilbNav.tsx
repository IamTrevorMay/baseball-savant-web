...
const PRIMARY_LINKS = [
  { href: '/milb/home', label: 'Home' },
  { href: '/milb/explore', label: 'Explore' },
  { href: '/milb/pitchers', label: 'Pitchers' },
  { href: '/milb/hitters', label: 'Hitters' },
  { href: '/milb/reports', label: 'Reports' },
]
...
export default function MilbNav({ active, children, rightContent }: Props) {
  ...
  return (
    <nav className="h-12 bg-zinc-900 border-b border-zinc-800 flex items-center px-4 md:px-6 relative shrink-0" ref={menuRef}>
      ...
      <a href="/milb/home" className="font-[family-name:var(--font-bebas)] text-amber-400 tracking-wide text-sm hover:text-amber-300 transition ml-4 hidden sm:inline">MiLB</a>
      <a href="/home" className="text-zinc-500 hover:text-zinc-300 text-xs transition ml-3 hidden sm:inline">MLB</a>
        ...
        <div className="flex gap-4 text-xs text-zinc-500">
          {PRIMARY_LINKS.map(link => (
            <a key={link.href} href={link.href}
              className={active === link.href ? 'text-amber-400' : 'hover:text-zinc-300 transition'}>
              {link.label}
            </a>
          ))}
        </div>
          ...
          {PRIMARY_LINKS.map(link => (
            <a key={link.href} href={link.href}
              className={`block px-6 py-3 text-sm border-t border-zinc-800/50 transition ${
                active === link.href ? 'text-amber-400 bg-zinc-800/30' : 'text-zinc-400 hover:text-white hover:bg-zinc-800/30'
              }`}>
              {link.label}
            </a>
          ))}

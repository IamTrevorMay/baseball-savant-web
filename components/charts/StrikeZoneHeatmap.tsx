...
export default function StrikeZoneHeatmap({ data }: { data: any[] }) {
  ...
  const trace = {
    ...
    customdata: f.map(d => [+(toPitcherX(d.plate_x) * 12).toFixed(1), +(d.plate_z * 12).toFixed(1)]),
  ...
  const layout = {
    ...
    xaxis: { ...BASE_LAYOUT.xaxis, title: PITCH_LOC_X_TITLE, range: [-2.5, 2.5], scaleanchor: 'y' },

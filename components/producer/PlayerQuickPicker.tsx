...
export default function PlayerQuickPicker({ type, onSelect, value, onClear, label }: Props) {
    ...
    const t = setTimeout(async () => {
      ...
      if (data) {
        setResults(data.map((d: any) => ({
          player_id: d.pitcher || d.batter || d.player_id,
    ...
    return (
      <div>
        ...
        <div className="flex items-center gap-2 h-9 px-3 bg-zinc-800 rounded border border-zinc-700">
          <span className="text-sm text-white flex-1 truncate">{value.player_name}</span>

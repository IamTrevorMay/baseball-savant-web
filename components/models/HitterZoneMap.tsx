...
function labelToColor(label: 'attack' | 'neutral' | 'avoid'): string {
  if (label === 'attack') return '#22c55e'
  if (label === 'avoid') return '#ef4444'
  return '#a1a1aa'
}

export function HitterZoneMap({
  zoneScores,
  batterZones,
}: {
  zoneScores: HitterZoneScore[]
  ...
  for (const zs of zoneScores) {
    ...
    x.push(coords.x)
    y.push(coords.z)
    colors.push(labelToColor(zs.label))
    sizes.push(zs.label === 'attack' ? 38 : zs.label === 'avoid' ? 28 : 22)
    ...
    <div className="relative w-full h-full">
      ...
      <div className="absolute bottom-1 left-1 flex items-center gap-3 bg-zinc-900/80 rounded px-2 py-1">
        ...
        <div className="flex items-center gap-1">
          ...
          <span className="text-[9px] text-zinc-400">Avoid</span>
        </div>
      </div>
    </div>
  )

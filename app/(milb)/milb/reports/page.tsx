...
function ReportsPageInner() {
  ...
  async function handleSubjectSearch(q: string) {
    ...
    const rpcName = subjectType === 'hitting' ? 'search_milb_batters' : 'search_milb_players'
    try {
      const { data, error } = await supabase.rpc(rpcName, { search_term: q.trim(), result_limit: 8 })
  ...
  async function handleModifierTargetSearch(q: string) {
    ...
    const rpcName = subjectType === 'hitting' ? 'search_milb_players' : 'search_milb_batters'
    try {
      const { data, error } = await supabase.rpc(rpcName, { search_term: q.trim(), result_limit: 8 })
  ...
  async function selectModifierTarget(p: any) {
    ...
    try {
      const res = await fetch(`/api/milb/player-data?id=${target.id}&col=${col}`)
  ...
  async function loadPlayerData(playerId: number) {
    ...
    try {
      const res = await fetch(`/api/milb/player-data?id=${playerId}&col=${col}`)
      const json = await res.json()
  ...
  async function loadTemplates() {
    const { data } = await supabase.from('milb_report_templates').select('id, name').order('created_at', { ascending: false })
  ...
  async function saveTemplate() {
    ...
    setSaving(true)
    await supabase.from('milb_report_templates').upsert({
  ...
  async function deleteTemplate(id: string) {
    await supabase.from('milb_report_templates').delete().eq('id', id)
    loadTemplates()
  }

  // Load template
  async function loadTemplate(id: string) {
    const { data } = await supabase.from('milb_report_templates').select('*').eq('id', id).single()
    if (data) {
  ...
  useEffect(() => {
    ...
    async function hydrateReport() {
      if (templateIdParam) {
        const { data: tmpl } = await supabase.from('milb_report_templates').select('*').eq('id', templateIdParam).single()
        if (tmpl) {
      ...
      if (modeParam === 'vs_similar_stuff' && type === 'pitching') {
        const pitcherRes = await fetch(`/api/milb/player-data?id=${playerId}&col=pitcher`)
        ...
        if (oppTypeParam === 'hitter' && oppIdParam) {
          const hitterRes = await fetch(`/api/milb/player-data?id=${oppIdParam}&col=batter`)
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-200">
      ...
      <div hidden={builderMode !== 'default'}>
        <ScoutingReportBuilder level="MiLB" modeToggle={modeToggle} active={builderMode === 'default'} />
      </div>

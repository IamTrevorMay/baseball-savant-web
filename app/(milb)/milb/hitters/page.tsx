...
export default function MilbHittersPage() {
  ...
  async function loadTopHitters() {
    const { data } = await supabase.rpc('search_milb_batters', { search_term: '', result_limit: 12 })
  ...
  function handleSearch(value: string) {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      setLoading(true)
      const { data } = await supabase.rpc('search_milb_batters', { search_term: value.trim(), result_limit: 8 })
  ...
  function goToHitter(batter: number) {
    router.push(`/milb/hitter/${batter}`)
  }

  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-200">
      {/* Nav */}
      <MilbNav active="/milb/hitters" rightContent={<span className="text-[11px] text-zinc-600 font-mono">{dbInfo.total.toLocaleString()} pitches</span>} />

      {/* Hero */}
      <div className="flex flex-col items-center justify-center pt-24 pb-16 px-4">
        <h1 className="text-4xl font-bold text-white mb-2">MiLB Hitter Search</h1>
        <p className="text-zinc-500 mb-10 text-sm">Search any MiLB hitter to view their complete Statcast dashboard</p>

...
const PARENT_ORG_COLORS: Record<number, string> = {
  ...
  147: '#003087', // NYY
  148: '#FFC52F', // MIL
  158: '#FFC52F', // MIL (alternate)
}

function getTeamColor(abbrev: string, parentOrgId: number | null): string {
  if (parentOrgId && PARENT_ORG_COLORS[parentOrgId]) {
    return PARENT_ORG_COLORS[parentOrgId]
  }
  return TEAM_COLORS[abbrev] || '#52525b'
}

const IL_ORDER = ['IL East', 'IL West']
const PCL_ORDER = ['PCL East', 'PCL West']

export default function MilbHomePage() {
  /* ─── Scores state ─── */
  ...
  const fetchScores = useCallback((date: string, showLoading = false) => {
    ...
    fetch(`/api/milb/scores?date=${date}`)
      .then(r => r.json())
  ...
  useEffect(() => {
    fetch('/api/milb/news').then(r => r.json()).then(d => { setNews(d.items || []); setNewsLoading(false) }).catch(() => setNewsLoading(false))
  }, [])

  useEffect(() => {
    setStandingsLoading(true)
    fetch(`/api/milb/standings?season=${season}`).then(r => r.json()).then(d => { if (d.divisions) setDivisions(d.divisions); setStandingsLoading(false) }).catch(() => setStandingsLoading(false))
  }, [season])

  function getDivisions(league: string) {
    const order = league === 'IL' ? IL_ORDER : PCL_ORDER
      ...
      <div className="max-w-7xl mx-auto px-6 py-6">
        ...
        <div className="mb-10">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-2xl font-bold text-white mb-1">AAA Scores</h2>
        ...
        <div className="mb-10">
          ...
          <p className="text-sm text-zinc-500 mb-6">Latest from around the minors</p>
        ...
        <div>
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              <h2 className="text-2xl font-bold text-white">AAA Standings</h2>
            </div>
            <div className="flex items-center gap-3">
          ...
          {standingsLoading ? (
            ...
            <>
              {view === 'division' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    <h3 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider">International League</h3>
                    {getDivisions('IL').map(div => <DivisionTable key={div.division} division={div} />)}
                  </div>
                  <div className="space-y-4">
                    <h3 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider">Pacific Coast League</h3>
                    {getDivisions('PCL').map(div => <DivisionTable key={div.division} division={div} />)}
                  </div>
                </div>
              )}
              ...
              {view === 'league' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div>
                    <h3 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider mb-4">International League</h3>
                    <LeagueTable teams={divisions.filter(d => d.league === 'IL').flatMap(d => d.teams).sort((a,b) => b.w - a.w || a.l - b.l)} />
                  </div>
                  <div>
                    <h3 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider mb-4">Pacific Coast League</h3>
                    <LeagueTable teams={divisions.filter(d => d.league === 'PCL').flatMap(d => d.teams).sort((a,b) => b.w - a.w || a.l - b.l)} />
                  </div>
                </div>
              )}
            </>

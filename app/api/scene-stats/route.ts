...
// Long-timeout variant (function-level statement_timeout=120s). The plain
// run_query RPC is capped at 8s by the authenticator role, which a
// season-wide GROUP BY over `pitches` cannot meet (~19s measured 2026-09-16),
// so every team-stats request that misses the MV fast path used to 500 with
  ...
  try {
    ...
    if (sp.get('teamStats') === 'true') {
      ...
      if (canUseMV) {
        ...
        // Must match the DEPLOYED mv_team_*_stats columns exactly. Both views
        // were widened 2026-09-16 (rebuild_team_stats_mvs(), generated by
        // scripts/gen-team-stats-mvs.ts from lib/reportMetrics.METRICS) so every
        // team-aggregatable SCENE_METRICS key hits this fast path. Regenerate
        // these lists from that script's output; verify with pg_attribute before
        // adding keys by hand — an unknown column here 500s the request.
        const mvPitchingCols: Record<string, boolean> = {
          team:true, game_year:true, pitches:true, games:true, pa:true, avg_velo:true,
          whiff_pct:true, k_pct:true, bb_pct:true, avg_xwoba:true, csw_pct:true, zone_pct:true,
          chase_pct:true, strikeouts:true, walks:true, hbp_count:true, home_runs:true, ip:true,
          xwoba_raw:true, woba_raw:true, max_velo:true, avg_spin:true, avg_hbreak_in:true,
          avg_ivb_in:true, avg_ext:true, avg_arm_angle:true, k_minus_bb:true, swstr_pct:true,
          contact_pct:true, z_swing_pct:true, o_contact_pct:true, ba:true, obp:true, slg:true,
          ops:true, avg_xba:true, avg_xslg:true, avg_woba:true, total_re24:true, avg_ev:true,
          max_ev:true, avg_la:true, avg_dist:true, hard_hit_pct:true, barrel_pct:true, gb_pct:true,
          fb_pct:true, ld_pct:true, iffb_pct:true, h:true, hr_count:true, k_count:true,
          bb_count:true, doubles:true, triples:true,
        }
        const mvBattingCols: Record<string, boolean> = {
          team:true, game_year:true, pitches:true, games:true, pa:true, runs:true, avg_ev:true,
          ba:true, slg:true, k_pct:true, bb_pct:true, avg_xwoba:true, hard_hit_pct:true,
          barrel_pct:true, woba_raw:true, whiff_pct:true, k_minus_bb:true, csw_pct:true,
          swstr_pct:true, zone_pct:true, chase_pct:true, contact_pct:true, z_swing_pct:true,
          o_contact_pct:true, obp:true, ops:true, avg_xba:true, avg_xslg:true, avg_woba:true,
          total_re24:true, max_ev:true, avg_la:true, avg_dist:true, gb_pct:true, fb_pct:true,
          ld_pct:true, iffb_pct:true, avg_bat_speed:true, avg_swing_length:true,
          avg_attack_angle:true, avg_attack_direction:true, avg_swing_path_tilt:true,
          fast_swing_rate:true, squared_up_rate:true, blast_rate:true,
          ideal_attack_angle_rate:true, h:true, hr_count:true, k_count:true, bb_count:true,
          doubles:true, triples:true, hbp_count:true,
        }
        ...
        if (allPitchesInMV) {
          ...
          const mvSelect = [...pitchesMetrics]
          if (statScope === 'hitting' && computedMetrics.includes('runs')) mvSelect.push('runs')
          const colList = mvSelect.length > 0 ? `, ${mvSelect.join(', ')}` : ''
          ...
          // Hitting 'runs' is selected from mv_team_batting_stats above (it was
          // documented as present but never selected before 2026-09-16). Pitching
          // scope has no runs-against column in the MV; it stays null.
      ...
      // Slow path (non-MV metric, date range, or SP/RP split): each query scans
      // a season of `pitches` (~20s), so use the 120s RPC.
      ...
      if (pitchesMetrics.length > 0) {
        ...
        if (starterIds) {
          ...
          `
          tasks.push((async () => {
            const { data, error: err } = await qLong(sql)
      ...
      if (eraMetrics.length > 0) {
        if (starterIds) {
          ...
          `
          tasks.push((async () => {
            const { data, error: err } = await qLong(eraSql)
            if (err) throw new Error(err.message)
      ...
      if (computedMetrics.includes('runs')) {
        ...
        if (starterIds) {
          ...
          `
          tasks.push((async () => {
            const { data, error: err } = await qLong(runsSql)
            if (err) throw new Error(err.message)
            for (const row of (data || []) as any[]) {
      ...
      if (computedMetrics.includes('wrc_plus')) {
        ...
        `
        tasks.push((async () => {
          const { data, error: err } = await qLong(wrcSql)
          if (err) throw new Error(err.message)
          const yr = parseInt(gameYear || String(new Date().getFullYear()))
          const constants = SEASON_CONSTANTS[yr] || SEASON_CONSTANTS[LATEST_SEASON_YEAR]
          for (const row of (data || []) as Record<string, any>[]) {

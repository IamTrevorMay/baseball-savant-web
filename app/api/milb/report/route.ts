...
const GROUP_COLS: Record<string, string> = {
  ...
  parent_org_away: 'parent_org_away',
...
const FILTER_COLS = new Set([
  ...
  'release_extension', 'effective_speed', 'spin_axis',
  'hit_distance_sc', 'home_score', 'away_score', 'n_thruorder_pitcher',
  'player_name', 'level', 'parent_org_home', 'parent_org_away',
])
...
export async function POST(req: NextRequest) {
  try {
    ...
    const sql = `SELECT ${selectParts.join(', ')} FROM milb_pitches ${whereClause} ${groupClause} ${havingClause} ${orderClause} ${limitClause}`

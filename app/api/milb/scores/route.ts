...
export async function GET(req: NextRequest) {
  ...
  try {
    const url = `https://statsapi.mlb.com/api/v1/schedule?date=${date}&sportId=11&hydrate=team,linescore,probablePitcher`
    ...
    const games = (dateEntry.games || []).map((g: any) => {
      ...
      return {
        ...
        away: {
          ...
          parentOrgId: away.team?.parentOrgId ?? null,
        },
        home: {
          id: home.team?.id,
          name: home.team?.name || '',
          ...
          parentOrgId: home.team?.parentOrgId ?? null,
        },

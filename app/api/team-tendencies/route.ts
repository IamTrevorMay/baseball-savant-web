  ...
  try {
    ...
    if (tab === 'bullpen') {
      const { data, error } = await q(`
        SELECT CASE WHEN inning_topbot = 'Top' THEN home_team ELSE away_team END as team,
          ...
          ${METRICS.avg_xwoba} as avg_xwoba
        FROM pitches
    ...
    const metricsSQL = isPitching
      ? `COUNT(*) as pitches,
         ...
         ${METRICS.bb_pct} as bb_pct,
         ${METRICS.avg_xwoba} as avg_xwoba,
         ${METRICS.csw_pct} as csw_pct,
         ${METRICS.zone_pct} as zone_pct,
         ${METRICS.chase_pct} as chase_pct`
      : `COUNT(*) as pitches,
         COUNT(DISTINCT game_pk) as games,
         ${METRICS.pa} as pa,
         ...
         ${METRICS.bb_pct} as bb_pct,
         ${METRICS.avg_xwoba} as avg_xwoba,
         ${METRICS.hard_hit_pct} as hard_hit_pct,
         ${METRICS.barrel_pct} as barrel_pct`

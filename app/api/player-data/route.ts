...
import { parsePlayerDataRows } from '@/lib/schemas/playerData'
...
export async function GET(req: NextRequest) {
  ...
  // Optional time period (years csv / legacy year / startDate / endDate) —
  // dramatically reduces rows (50K → ~5K per season)
  const parsed = parsePeriodParams(req.nextUrl.searchParams)
  if ('error' in parsed) return NextResponse.json({ error: parsed.error }, { status: 400 })
  const periodFilter = periodWhereSql(parsed.period, 'p.')
  ...
  try {
    ...
    const sql = `SELECT ${prefixedColumns}, pl.name as ${nameCol}_name FROM pitches p LEFT JOIN players pl ON pl.id = p.${nameCol} WHERE p.${col} = ${safeId} AND COALESCE(p.pitch_type, '') NOT IN ('PO','IN')${periodFilter} ORDER BY p.game_date DESC LIMIT 50000`
    ...
    const validated = data ? parsePlayerDataRows(data) : []

    return NextResponse.json({ rows: validated, count: validated.length }, {

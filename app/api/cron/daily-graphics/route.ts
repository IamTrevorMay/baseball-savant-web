...
async function generateTrends(): Promise<any> {
  ...
  async function fetchAlerts(playerType: 'pitcher' | 'hitter') {
    ...
    const PITCHER_METRICS: MetricDef[] = [
      ...
      { key: 'k_pct', label: 'K%', higherIsBetter: true, seasonSQL: "ROUND(100.0 * COUNT(*) FILTER (WHERE events LIKE '%strikeout%') / NULLIF(COUNT(DISTINCT CASE WHEN events IS NOT NULL AND events <> 'truncated_pa' THEN game_pk::bigint * 10000 + at_bat_number END), 0), 1)", recentSQL: "ROUND(100.0 * COUNT(*) FILTER (WHERE game_date >= '{recent}' AND events LIKE '%strikeout%') / NULLIF(COUNT(DISTINCT CASE WHEN events IS NOT NULL AND events <> 'truncated_pa' AND game_date >= '{recent}' THEN game_pk::bigint * 10000 + at_bat_number END), 0), 1)" },
    ...
    const HITTER_METRICS: MetricDef[] = [
      ...
      { key: 'k_pct', label: 'K%', higherIsBetter: false, seasonSQL: "ROUND(100.0 * COUNT(*) FILTER (WHERE events LIKE '%strikeout%') / NULLIF(COUNT(DISTINCT CASE WHEN events IS NOT NULL AND events <> 'truncated_pa' THEN game_pk::bigint * 10000 + at_bat_number END), 0), 1)", recentSQL: "ROUND(100.0 * COUNT(*) FILTER (WHERE game_date >= '{recent}' AND events LIKE '%strikeout%') / NULLIF(COUNT(DISTINCT CASE WHEN events IS NOT NULL AND events <> 'truncated_pa' AND game_date >= '{recent}' THEN game_pk::bigint * 10000 + at_bat_number END), 0), 1)" },
      { key: 'bb_pct', label: 'BB%', higherIsBetter: true, seasonSQL: "ROUND(100.0 * COUNT(*) FILTER (WHERE events IN ('walk','intent_walk')) / NULLIF(COUNT(DISTINCT CASE WHEN events IS NOT NULL AND events <> 'truncated_pa' THEN game_pk::bigint * 10000 + at_bat_number END), 0), 1)", recentSQL: "ROUND(100.0 * COUNT(*) FILTER (WHERE game_date >= '{recent}' AND events IN ('walk','intent_walk')) / NULLIF(COUNT(DISTINCT CASE WHEN events IS NOT NULL AND events <> 'truncated_pa' AND game_date >= '{recent}' THEN game_pk::bigint * 10000 + at_bat_number END), 0), 1)" },

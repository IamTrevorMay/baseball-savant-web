...
export default function DailyCardsPage() {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-200">
      <ResearchNav active="/daily-cards" />
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  disabled={exporting}
                  className="px-3 py-1.5 rounded text-xs font-medium bg-emerald-600 text-white hover:bg-emerald-500 transition disabled:opacity-40 flex items-center gap-1.5"
                >
                ...
                {cards.map(card => (
                  ...
                  >
                    ...
                    <div className="relative overflow-hidden bg-zinc-950" style={{ height: 200 }}>
                      ...
                      {/* Rank badge */}
                      <div className="absolute top-2 left-2 bg-emerald-600 text-white text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center">
                        {card.rank}
                      </div>
                    </div>
                    {/* Info */}
              ...
              {archive.map(entry => (
                <button key={entry.date} onClick={() => { setCurrentDate(entry.date); setSelectedCard(null) }}
                  className={`text-left bg-zinc-900 border rounded-lg p-4 hover:border-zinc-700 hover:bg-zinc-800/50 transition ${
                    entry.date === currentDate ? 'border-emerald-500 ring-1 ring-emerald-500/30' : 'border-zinc-800'
                  }`}>
                  ...
                  <div className="space-y-1">
                    {entry.pitchers.map((p, i) => (
                      <div key={i} className="flex items-center gap-2 text-xs">
                        <span className="text-emerald-500 font-bold w-3">{p.rank}</span>

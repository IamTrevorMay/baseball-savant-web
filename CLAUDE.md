...
- **FanGraphs-standard rate stats** (adopted 2026-09-11): PA/TBF excludes `truncated_pa`; BB includes `intent_walk` (except inside wOBA formulas, which use unintentional BB); AB = PA − BB − HBP − SF − SH − CI; FB% folds popups in (GB+LD+FB = 100%) and `iffb_pct` (popups ÷ all fly balls) replaces `pu_pct`. Shared SQL fragments at the top of `lib/reportMetrics.ts`. The DEPLOYED database functions/MVs are the source of truth — repo `scripts/*.sql` are synced copies (drift there has caused silent production bugs).

...
`mv_team_pitching_stats`, so `/api/scene-stats?teamStats=true` fell to a season-wide
`pitches` aggregate (~19s) through `run_query` (8s cap). Fixes: (1) the slow path
(custom date range, SP/RP split, or any non-MV metric) now uses `run_query_long`
(120s) instead of erroring; (2) `mv_team_pitching_stats` (20 → 55 cols) and
`mv_team_batting_stats` (15 → 53 cols) now carry every team-aggregatable
`SCENE_METRICS` key — existing columns verbatim from the deployed views, new ones
from `lib/reportMetrics.METRICS` so fast and slow paths agree. DDL is generated by
`scripts/gen-team-stats-mvs.ts` → `scripts/create-team-stats-mvs.sql`
(`rebuild_team_stats_mvs()`: builds `_new` views alongside, swaps at the end, run as a
one-shot pg_cron job). Route allowlists updated; hitting `runs` is now actually
selected on the fast path. `refresh_materialized_views()` still refreshes both by
name. NB `rebuild_savant_mvs()` still contains the OLD narrow team-view DDL.

...
**Not done (deferred v2):** force-plate ingest + inverse-dynamics kinetics, regression torque estimate, PULSE workload / A:C ratio, mph-per-torque efficiency index. ~~3D skeleton playback~~ ✓ shipped Aug 2026, below.

### MEchanics — Capture Reviewer + Synthetic C3D (August 2026)
Closed the gap the v1 audit found: every capture in the DB was seeded by `seed-mechanics-demo.ts`, so `parseC3D`, the Captury label mapper and event detection had **never run on a file**. Full write-up in `docs/mechanics.md`.

**Shipped:**
- **Synthetic Captury generator** (`scripts/generate-mechanics-c3d.ts` + `scripts/mechanics-demo/`) — parameterised forward-kinematics delivery model → real `.c3d` (Intel float, Z-up, mm, MotionBuilder/HIK names), 8-throw bullpens, 2 athletes × 3 sessions. A calibrator bisects pose parameters **against the real extractor**, so a target percentile is requested, never asserted. Output validated against the independent Python `c3d` library and against rigid-body/anatomical invariants (0.00% bone-length variation at zero noise). Files land in the gitignored `data/mechanics-demo/` with a `MANIFEST.md` of upload settings.
- **3D capture reviewer** (`components/mechanics/SkeletonViewer.tsx`, `lib/mechanics/skeleton.ts`) — react-three-fiber playback on the capture-detail page. Raw C3D via a 5-min signed URL (`GET /api/mechanics/captures/[id]/raw`), parsed **client-side with the same pipeline libs**, so the skeleton is the geometry the metrics were computed from. Event snapping (FC/MER/release) + frame stepping + throw selector, measurement overlays that draw the exact lines metrics.ts takes its angles from, session-over-session ghost aligned at foot contact, and camera presets locked to the measurement planes. Playback is driven by a frame ref in `useFrame` — no React re-render at 240 fps.
- **Kinematic sequence chart** (`components/mechanics/KinematicSequence.tsx`, `lib/mechanics/sequence.ts`) — hips/chest/upper-arm/hand angular velocity on one °/s axis with lead-knee extension in its own band, playhead synced to the 3D view. Uses the identical heading-rate math *and each metric's own search interval*, verified peak-for-peak against `extractThrowMetrics`. Smoothed line over the raw derivative, so the unfiltered noise is visible rather than hidden.

**Metric-definition problems this surfaced** (all in `METRIC_DEFS` vs `metrics.ts`, none fixed — they change every stored report's numbers): `leadKneeExtVelocity` is a window mean compared against a *peak* norm band; `strideLengthPct` measures instantaneous inter-ankle distance and cannot reach its 85%-of-height norm; `peakAngVel` differentiates raw positions with no low-pass, so velocity metrics inflate with marker noise (p50→p91 at 3 mm); trunk tilt is split along L1 so a true 35°/20° lean reports as 25.2°/13.1°; `shoulderIrVelocity` is a horizontal heading rate — ill-conditioned and not reliably ordinal.
...
**2026-07-10 — Videos page synced with Mayday Studio's Pitch Video Search.** Ported the four post-launch updates from Mayday Studio (`d4110ecb`…`2aa2d6e9`) into `app/(research)/videos/page.tsx`: (1) unindexed rows now labeled "not archived" (status is null pre-2026); (2) **Playlist view** — personal DB-backed playlists (`pitch_playlists` + `pitch_playlist_items`, RLS owner-only, items snapshot the pitch row as jsonb; migration `pitch_playlists`, DDL in `scripts/create-pitch-playlists.sql`) with a frame-step player (¼/½/1/2× speed, 1/30s arrow-key frame stepping, ↑↓ clip nav, auto-advance toggle); (3) any pitch selectable — playlists/downloads live-resolve the Savant CDN mp4 for unarchived rows; (4) multi-clip batch downloads bundle into one store-mode zip via `fflate` (browsers block programmatic downloads after the first). Backfill progress at time of port: ~174k/537k clips downloaded (32%).

**2026-08-26 — 35-day download outage found and fixed (UA 403).** The archive had not gained a clip since **2026-07-22** while indexing stayed current, so the failure was invisible from the Videos page. Cause: `sporty-clips.mlb.com` began 403ing the worker's bare `'Mozilla/5.0'` User-Agent in late July (the `sporty-videos` page itself never cared, so clip URLs kept resolving and every download failed). Verified with curl on a live play_id: bare UA 403, full Chrome UA 200, `Referer` irrelevant. **Blast radius was multiplied by the retry design** — `MAX_ATTEMPTS = 6` plus a nightly job burned six attempts in six nights, settling **134,284 real clips as terminal `missing`** ("Savant has no clip"), which is exactly wrong. Fixes in `scripts/download-pitch-videos.ts`: (1) full browser UA + `Referer` on both fetches; (2) a blocked/5xx error (`BLOCKED_RE`) no longer increments `attempts` and can never reach `missing` — the row parks in `failed` indefinitely so a visible backlog replaces a silent write-off; (3) the `--include-failed` claim filter also picks up blocked rows parked at `attempts >= MAX_ATTEMPTS`. **153,152 rows requeued** to `pending` (`WHERE error = 'mp4 fetch 403'`); verification run downloaded 3/3 clips (5.6–8.1 MB, valid mp4s). The nightly pm2 `pitch-video-worker` drains the backlog from here — at the historical ~26k clips/week that is roughly six weeks, so consider a manual high-concurrency catch-up run.

**Monitoring gap this exposed — now closed.** `checkPitchVideoArchive()` (`lib/dataIntegrity.ts`, check 9) is a dead-man switch on `max(downloaded_at)`, wired into the daily integrity cron: **pass < 48h, warn 48–96h, fail > 96h**, with a Resend email on fail (`INTEGRITY_NOTIFY_EMAIL`, falling back to `JANITOR_NOTIFY_EMAIL`; log-only if neither is set). Two design points worth keeping: staleness only counts when work is actually queued, so a caught-up archive never cries wolf; and the queue is `pending + failed`, because the outage parked rows in `failed` and a dead-man watching only `pending` would have stayed silent through the whole thing. The alert is scoped to this check alone — the other eight fail only when their promise throws, and emailing on those would train the inbox to ignore the address. 7 unit tests in `__tests__/lib/dataIntegrity.archive.test.ts` cover the thresholds, the empty-queue silence, and the failed-rows-count-as-queued case. Verified against live data: `pass`, backlog 153,133, last download seconds prior.

**2026-08-26 — catch-up run started.** pm2 app `pitch-video-catchup` (`--concurrency 6`, `--no-autorestart`), 537 games / 153,149 clips ≈ 2–3 days, ~800GB at ~5MB/clip against 41TB free. The PID lock makes the 4am nightly defer to it. Remove the app once it exits; the nightly worker resumes normal drain duty.

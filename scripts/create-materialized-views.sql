...
SELECT
  ...
  ROUND(100.0 * COUNT(*) FILTER (WHERE p.events IN ('walk','intent_walk'))
    / NULLIF(COUNT(DISTINCT CASE WHEN p.events IS NOT NULL AND p.events NOT IN ('truncated_pa','game_advisory','ejection','wild_pitch','passed_ball','other_advance','runner_double_play','caught_stealing_2b','caught_stealing_3b','caught_stealing_home','pickoff_1b','pickoff_2b','pickoff_3b','pickoff_caught_stealing_2b','pickoff_caught_stealing_3b','pickoff_caught_stealing_home','stolen_base_2b','stolen_base_3b','stolen_base_home') THEN p.game_pk::bigint * 10000 + p.at_bat_number END), 0), 1) AS bb_pct,
  -- Whiff% / CSW% / SwStr%
  ROUND(100.0 * COUNT(*) FILTER (WHERE p.description LIKE '%swinging_strike%' OR p.description IN ('missed_bunt','swinging_pitchout','foul_tip','bunt_foul_tip'))
  ...
  ROUND(100.0 * COUNT(*) FILTER (WHERE p.zone BETWEEN 1 AND 9 AND (p.description LIKE '%swinging_strike%' OR p.description LIKE '%foul%' OR p.description LIKE 'hit_into_play%' OR p.description = 'missed_bunt' OR p.description = 'swinging_pitchout'))
  ...
  ROUND(COUNT(*) FILTER (WHERE p.events IN ('single','double','triple','home_run'))::numeric
    / NULLIF(COUNT(*) FILTER (WHERE p.events IN ('single','double','triple','home_run','field_out','strikeout','strikeout_double_play','grounded_into_double_play','force_out','double_play','field_error','fielders_choice','fielders_choice_out','triple_play','other_out')), 0), 3) AS ba,
  ...
  ROUND((COUNT(*) FILTER (WHERE p.events IN ('single','double','triple','home_run','walk','intent_walk','hit_by_pitch')))::numeric
    / NULLIF(COUNT(*) FILTER (WHERE p.events IN ('single','double','triple','home_run','field_out','strikeout','strikeout_double_play','grounded_into_double_play','force_out','double_play','field_error','fielders_choice','fielders_choice_out','triple_play','other_out','walk','intent_walk','hit_by_pitch','sac_fly','sac_fly_double_play')), 0), 3) AS obp,
  -- Expected
  ROUND(((COALESCE(SUM(p.estimated_woba_using_speedangle) FILTER (WHERE p.description LIKE 'hit_into_play%'), 0) + 0.7 * COUNT(*) FILTER (WHERE p.events = 'walk') + 0.7 * COUNT(*) FILTER (WHERE p.events = 'hit_by_pitch')) / NULLIF(COUNT(*) FILTER (WHERE p.events IN ('single','double','triple','home_run','field_out','strikeout','strikeout_double_play','grounded_into_double_play','force_out','double_play','field_error','fielders_choice','fielders_choice_out','triple_play','other_out','walk','hit_by_pitch','sac_fly','sac_fly_double_play')), 0))::numeric, 3) AS avg_xwoba,
  ROUND(SUM(p.estimated_ba_using_speedangle)::numeric
    / NULLIF(COUNT(*) FILTER (WHERE p.events IN ('single','double','triple','home_run','field_out','strikeout','strikeout_double_play','grounded_into_double_play','force_out','double_play','field_error','fielders_choice','fielders_choice_out','triple_play','other_out')), 0), 3) AS avg_xba,
  ROUND(AVG(p.estimated_slg_using_speedangle)::numeric, 3) AS avg_xslg,
  ROUND(((0.7 * COUNT(*) FILTER (WHERE p.events = 'walk') + 0.7 * COUNT(*) FILTER (WHERE p.events = 'hit_by_pitch') + 0.9 * COUNT(*) FILTER (WHERE p.events = 'single') + 1.25 * COUNT(*) FILTER (WHERE p.events = 'double') + 1.6 * COUNT(*) FILTER (WHERE p.events = 'triple') + 2.0 * COUNT(*) FILTER (WHERE p.events = 'home_run')) / NULLIF(COUNT(*) FILTER (WHERE p.events IN ('single','double','triple','home_run','field_out','strikeout','strikeout_double_play','grounded_into_double_play','force_out','double_play','field_error','fielders_choice','fielders_choice_out','triple_play','other_out','walk','hit_by_pitch','sac_fly','sac_fly_double_play')), 0))::numeric, 3) AS avg_woba,
  -- Batted ball quality
  ROUND(AVG(p.launch_speed) FILTER (WHERE p.bb_type IS NOT NULL)::numeric, 1) AS avg_ev,
  ROUND(MAX(p.launch_speed) FILTER (WHERE p.bb_type IS NOT NULL)::numeric, 1) AS max_ev,
  ROUND(AVG(p.launch_angle) FILTER (WHERE p.bb_type IS NOT NULL)::numeric, 1) AS avg_la,
  ...
  ROUND(100.0 * COUNT(*) FILTER (WHERE p.launch_speed_angle::text = '6')
    / NULLIF(COUNT(*) FILTER (WHERE p.bb_type IS NOT NULL), 0), 1) AS barrel_pct,
  -- GB/FB/LD
...
SELECT
  ...
  ROUND(AVG(p.estimated_slg_using_speedangle)::numeric, 3) AS avg_xslg,
  ROUND(((0.7 * COUNT(*) FILTER (WHERE p.events = 'walk') + 0.7 * COUNT(*) FILTER (WHERE p.events = 'hit_by_pitch') + 0.9 * COUNT(*) FILTER (WHERE p.events = 'single') + 1.25 * COUNT(*) FILTER (WHERE p.events = 'double') + 1.6 * COUNT(*) FILTER (WHERE p.events = 'triple') + 2.0 * COUNT(*) FILTER (WHERE p.events = 'home_run')) / NULLIF(COUNT(*) FILTER (WHERE p.events IN ('single','double','triple','home_run','field_out','strikeout','strikeout_double_play','grounded_into_double_play','force_out','double_play','field_error','fielders_choice','fielders_choice_out','triple_play','other_out','walk','hit_by_pitch','sac_fly','sac_fly_double_play')), 0))::numeric, 3) AS avg_woba,
  ((0.7 * COUNT(*) FILTER (WHERE p.events = 'walk') + 0.7 * COUNT(*) FILTER (WHERE p.events = 'hit_by_pitch') + 0.9 * COUNT(*) FILTER (WHERE p.events = 'single') + 1.25 * COUNT(*) FILTER (WHERE p.events = 'double') + 1.6 * COUNT(*) FILTER (WHERE p.events = 'triple') + 2.0 * COUNT(*) FILTER (WHERE p.events = 'home_run')) / NULLIF(COUNT(*) FILTER (WHERE p.events IN ('single','double','triple','home_run','field_out','strikeout','strikeout_double_play','grounded_into_double_play','force_out','double_play','field_error','fielders_choice','fielders_choice_out','triple_play','other_out','walk','hit_by_pitch','sac_fly','sac_fly_double_play')), 0)) AS woba_raw,  -- unrounded for wRC+ precision
  -- Batted ball quality
  ROUND(AVG(p.launch_speed) FILTER (WHERE p.bb_type IS NOT NULL)::numeric, 1) AS avg_ev,
  ROUND(MAX(p.launch_speed) FILTER (WHERE p.bb_type IS NOT NULL)::numeric, 1) AS max_ev,
  ROUND(AVG(p.launch_angle) FILTER (WHERE p.bb_type IS NOT NULL)::numeric, 1) AS avg_la,
...
SELECT
  ...
  ROUND((COUNT(DISTINCT CASE WHEN p.events IS NOT NULL AND p.events NOT IN ('single','double','triple','home_run','walk','hit_by_pitch','catcher_interf','field_error') THEN p.game_pk::bigint * 10000 + p.at_bat_number END)
   + COUNT(DISTINCT CASE WHEN p.events LIKE '%double_play%' THEN p.game_pk::bigint * 10000 + p.at_bat_number END)
   + 2 * COUNT(DISTINCT CASE WHEN p.events = 'triple_play' THEN p.game_pk::bigint * 10000 + p.at_bat_number END))::numeric / 3.0, 1) AS ip,
  ((COALESCE(SUM(p.estimated_woba_using_speedangle) FILTER (WHERE p.description LIKE 'hit_into_play%'), 0) + 0.7 * COUNT(*) FILTER (WHERE p.events = 'walk') + 0.7 * COUNT(*) FILTER (WHERE p.events = 'hit_by_pitch')) / NULLIF(COUNT(*) FILTER (WHERE p.events IN ('single','double','triple','home_run','field_out','strikeout','strikeout_double_play','grounded_into_double_play','force_out','double_play','field_error','fielders_choice','fielders_choice_out','triple_play','other_out','walk','hit_by_pitch','sac_fly','sac_fly_double_play')), 0)) AS xwoba_raw,
...
BEGIN
  ...
  SELECT
    ...
    ROUND(COUNT(*) FILTER(WHERE p.events IN ('single','double','triple','home_run','walk','intent_walk','hit_by_pitch'))::numeric/NULLIF(COUNT(DISTINCT CASE WHEN p.events IS NOT NULL AND p.events NOT IN ('sac_bunt','catcher_interf') THEN p.game_pk::bigint*10000+p.at_bat_number END),0),3),
